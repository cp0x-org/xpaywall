...
- You are logged in as `alice` and your project slug is `demo`, so the proxy path is `/alice/demo/...` (format: `/{username}/{slug}/{path}`).
  ...
  "accepts": [
    {
      ...
      "resource": "http://localhost:3102/alice/demo/weather",
...
```bash
curl -i http://localhost:3102/alice/demo/health
```
...
```bash
curl -i http://localhost:3102/alice/demo/no-such-path
```
