...
So if the project's base URL is `https://api.example.com` and a route's path is `/forecast`, calls to `https://gateway.example.com/<username>/<slug>/forecast` are forwarded to `https://api.example.com/forecast`. Any query string the client sent is appended unchanged.
...
> **Screenshot placeholder:** ![Project form](./../images/projects-form.png "medium")
...
```bash
curl -i http://localhost:3102/<username>/<slug>/<some-existing-route>
```
