...
- **Slug:** pick a short identifier. **This becomes part of the URL** your clients call (`/<username>/<slug>/...`), so once you publish it, you cannot rename it without breaking clients. The slug is unique per user, so a short slug like `default` is fine. If you want client-facing URLs close to your old ones, pick a slug that matches the prefix you were using.
- **Auth Header Name / Value:** the YAML `auth_header` fields.
- **Allow Unmatched Routes:** mirror the YAML `allow_unmatched` flag.

> **URLs change.** In file mode the proxy URL was usually `http://gateway/<path>`. In HTTP mode it becomes `http://gateway/<username>/<slug>/<path>` — both your login username and the project slug are now prefixes. If you cannot afford to change client URLs, see *Keeping URLs identical* at the end of this guide.
...
The biggest behavioural change is the owner username and project slug appearing in the path. There are two ways to avoid it:

1. **Reverse proxy rewrite.** Put a small nginx in front of xgateway that strips the leading client-facing prefix or adds the `/<username>/<slug>` prefix. Clients keep their old URL; xgateway sees the new one.
2. **Run xgateway with a single project and treat the prefix as silent.** If you have exactly one project and operate a separate xgateway instance per upstream, the `/<username>/<slug>` prefix is purely cosmetic. Choose short ones and document the URL change once.
